...
# distinct: loc trung ca hang
# jsonFile.select(col("payload.forkee.id")).distinct().selectExpr("count(*) as count_id").show()
# jsonFile.select(col("payload.forkee.id")).distinct().selectExpr("count(id) as count_id1").show()

# dropDuplicates: loc theo cac cot liet ke
jsonFile.select(col("payload.forkee.id")).alias("id").drop_duplicates(["id"]).show()