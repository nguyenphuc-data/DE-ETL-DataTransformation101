...
from pyspark.sql.functions import col, upper, length, lit
...
- col(), lit(): de xac dinh gia tri cua cot
"""

# jsonFile.withColumn("id2",lit("phuc")).select(col("actor.id"),col("id2")).show()

def phuc(data : int) -> int:
    return data + 2;

df = jsonFile.withColumn("")