...
# jsonFile.select(col("id")).sort(col("id")).show()

# jsonFile.select(col("id")).orderBy(col("id").desc()).show()

# Sap xep nhieu hang, nhieu cot
jsonFile.select(col("id"),col("actor.id")).orderBy([col("id"),col("actor.id")],ascending = [True,False]).show()